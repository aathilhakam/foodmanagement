...
const PostManagement = () => {
  ...
  if (!user || (user.role !== "shop_admin" && user.role !== "admin")) return navigate("/dashboard");
  ...
  // Filter posts based on user role and status
  ...
  const [newPost, setNewPost] = useState({
    ...
    image: "",
    isPinned: false,
    status: "published", // Add status field: draft, published, closed
  });
  ...
  const handleEditPost = (post) => {
    // Allow admins to edit any post, otherwise only own posts
    if (user.role !== "admin" && post.author !== user.name) {
  ...
  const handleUpdatePost = () => {
    ...
    setEditingPost(null);
    setNewPost({ title: "", content: "", excerpt: "", tags: [], image: "", isPinned: false, status: "published" });
  };

  const handleDeletePost = (postId) => {
    const postToDelete = posts.find(p => p.id === postId);
    // Allow admins to delete any post, otherwise only own posts
    if (user.role !== "admin" && postToDelete.author !== user.name) {
      return; // Don't allow deletion
    }
  ...
  const handleTogglePin = (postId) => {
    ...
    // Allow admins to pin any post, otherwise only own posts
    if (user.role !== "admin" && postToPin.author !== user.name) {
      return; // Don't allow pinning
    }
        ...
        <Card className="mb-8">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center gap-2 mb-4">
              ...
              />
              <label htmlFor="pin-post" className="text-sm font-medium">
                Pin this post
              </label>
            </div>
              ...
              <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={post.status === "published" ? "default" : "secondary"} className="ml-2">
                        {post.status === "published" ? "Published" : post.status === "draft" ? "Draft" : "Closed"}
                      </Badge>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-2 ml-4">
                  <Button
                    ...
                    disabled={user.role !== "admin" && post.author !== user.name}
                  >
