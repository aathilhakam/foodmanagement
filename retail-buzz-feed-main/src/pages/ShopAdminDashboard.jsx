...
import { offers as seedOffers, shops as seedShops, blogPosts as seedBlogPosts, comments } from "@/data/mockData";
...
const ShopAdminDashboard = () => {
  ...
  const handleToggleCanteenStatus = () => {
    ...
    // Update the shop status in localStorage
    const currentShops = storage.get(STORAGE_KEYS.SHOPS, seedShops);
    const updatedShops = currentShops.map((s) => 
      s.id === user.shopId ? { ...s, status: newStatus } : s
    );
    storage.set(STORAGE_KEYS.SHOPS, updatedShops);
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              className="bg-card/40"
            />
            <Input type="date" value={newOffer.validUntil} onChange={(e) => setNewOffer((p) => ({ ...p, validUntil: e.target.value }))} className="bg-card/40" />
          </div>
        </div>
